...
class PasswordFromTest < PasswordResetForm
  ...
  test "reset whith wrong email" do
...
class PasswordUpdateTest < PasswordResetForm
  test "update with invalid password and confirmation" do
    patch password_reset_path(@reset_user.reset_token),
      params: { email: @reset_user.email, user: {
        password: 'foobaz', password_confirmation: 'barguux' }}
...
class ExpiredToken < PasswordResets
  ...
  def setup
    ...
    post password_resets_path, params: { password_reset: {
      email: @user.email }}
    @reset_user = assigns(:user)
    @reset_user.update_attribute(:reset_send_at, 3.hours.ago)
